...
volatile uint64_t tohost = 0x800838e8;
volatile uint64_t fromhost = 0x800838e0;

volatile unsigned long *to_host = 0x800838e8;
volatile unsigned long *from_host = 0x800838e0;
...
{
	while (__read_tohost()){
		//asm("li t5, 0x800838e8; li t6, 0x101000000000000; ori t6, t6, 0x50; sd t6, (t5)");
		__check_fromhost();
	}
	//asm("li t5, 0x800838e8; li t6, 0x101000000000000; ori t6, t6, 0x51; sd t6, (t5)");
...
static void htif_putc(char ch)
{
	unsigned long pmpcfg_orig = csr_read(CSR_PMPCFG0);
	unsigned char _reset_required = 0;
	// if((pmpcfg_orig & 0x3000000) != 0x3000000){
	// 	register unsigned long a0 asm("a0") = 0x3000000;
	// 	asm("csrs pmpcfg0, a0" : : "r"(a0));
	// 	_reset_required = 1;
	// }
	...
	// if(_reset_required){
	// 	register unsigned long a0 asm("a0") = pmpcfg_orig;
	// 	asm("csrs pmpcfg0, a0" : : "r"(a0));
	// }

	//lets put set and reset into sbi_printf to call it less often...
		     ...
		     unsigned long custom_tohost_addr)
{
	...
	//asm("li t5, 0x800838e8; li t6, 0x101000000000000; ori t6, t6, 0x40; sd t6, (t5)");
	//rc = set_custom_addr(custom_addr, custom_fromhost_addr,
	//		     custom_tohost_addr);
	//asm("li t5, 0x800838e8; li t6, 0x101000000000000; ori t6, t6, 0x52; sd t6, (t5)");
	// if (rc)
	// 	mon_close_osbi();
	// 	return rc;
	...
	//mon_close_osbi();
	//asm("li t5, 0x800838e8; li t6, 0x101000000000000; ori t6, t6, 0x53; sd t6, (t5)");
	//htif_putc(0x4F);
	return 0;
}
