...
int pmp_unset_global(int region_idx)
{
  ...
  mon_send_sync_ipi(region_idx, 1, PMP_NO_PERM);
...
int pmp_set_global(int region_idx, uint8_t perm)
{
  ...
  mon_send_sync_ipi(region_idx, 0, perm);
...
void epmp_rlb(){
  //csr_write(CSR_MSECCFG, 0b100);
  csr_clear(CSR_PMPADDR6, 0x0); //Dummy OP if mseccfg not available
  asm("fence");
}

void epmp_activate(){
  //csr_write(CSR_MSECCFG, 0b101);
  csr_clear(CSR_PMPADDR6, 0x0); //Dummy OP if mseccfg not available
  asm("fence");
}
...
int pmp_set_keystone(int region_idx, uint8_t perm)
{
  ...
  uint8_t perm_bits = perm & EPMP_ALL_ALL_PERM;
  ...
  // unsigned long read_pmpcfg = csr_read(CSR_PMPCFG0);
  // //read csr and ckec the first byte if it has rw for m mode so priunt does not crash
  // if(((read_pmpcfg >> 1) & 0b1111) == 0b1111){
  ...
  switch(priority)
  {
    case(PMP_PRI_ANY): {
      ...
      if(TEST_BIT(reg_bitmap, reg_idx) || reg_idx >= PMP_N_REG){
        //PMP_ERROR(SBI_ERR_SM_PMP_REGION_MAX_REACHED, "PMP register unavailable");
        sbi_printf("%s:" "PMP register unavailable" "\n", __func__);
        sbi_printf("[SM] Bitmap: 0x%x, Index: 0x%x\n", reg_bitmap, reg_idx);
        return SBI_ERR_SM_PMP_REGION_MAX_REACHED;
      }
      break;
    }
    case(PMP_PRI_TOP): {
