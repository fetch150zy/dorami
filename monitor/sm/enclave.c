...
static inline void context_switch_to_enclave(struct sbi_trap_regs* regs,
  ...
  for(memid=0; memid < ENCLAVE_REGIONS_MAX; memid++) {
    if(enclaves[eid].regions[memid].type != REGION_INVALID) {
      pmp_set_keystone(enclaves[eid].regions[memid].pmp_rid, (PMP_X | PMP_W | PMP_R));
    }
  }

  //osm_pmp_set(PMP_NO_PERM); //It sometimes uses the last one for something else...
  ...
  //sbi_printf("SET ENCLAVE CONTEXT, switching now\n");
}
...
static inline void context_switch_to_host(struct sbi_trap_regs *regs,
  ...
  //sbi_printf("RESTORED HOST CONTEXT, switching now...\n");

  return;
}

unsigned long enclave_fix_access_permissions(enclave_id eid){
  osm_pmp_set(PMP_NO_PERM);
  int memid;
  for(memid=0; memid < ENCLAVE_REGIONS_MAX; memid++) {
    if(enclaves[eid].regions[memid].type != REGION_INVALID) {
...
unsigned long create_enclave(unsigned long *eidptr, struct keystone_sbi_create_t create_args)
{
  ...
  // pmp_set_keystone(region, PMP_NO_PERM);
  // pmp_set_keystone(shared_region, PMP_NO_PERM);

  asm("fence.i");

  // cleanup some memory regions for sanity See issue #38
  //pmp_set_keystone(shared_region, EPMP_ALL_RW);
  //osm_pmp_set(PMP_X | PMP_W); //SMAP-unlock
  osm_pmp_set(PMP_R | PMP_W); //SMAP-unlock
  clean_enclave_memory(utbase, utsize);
  osm_pmp_set(PMP_X | PMP_W | PMP_R); //SMAP-unlock
  //pmp_set_keystone(shared_region, PMP_X);
  ...
  pmp_set_keystone(region, EPMP_ALL_RW);
  //pmp_set_keystone(shared_region, EPMP_ALL_RW);
  //osm_pmp_set(PMP_X | PMP_W); //SMAP-unlock
  osm_pmp_set(PMP_R | PMP_W); //SMAP-unlock
  ...
  *eidptr = eid;
  ...
  //sbi_printf("ENCLAVE INITIALIZED AND MEASURED\n");
...
unsigned long destroy_enclave(enclave_id eid)
{
  ...
  for(i = 0; i < ENCLAVE_REGIONS_MAX; i++){
    ...
    pmp_set_keystone(rid, EPMP_ALL_RW);
...
static unsigned long ocall_info[5] = {0};
...
unsigned long stop_enclave(struct sbi_trap_regs *regs, uint64_t request, enclave_id eid)
{
  ...
  // if(request == STOP_EDGE_CALL_HOST){
  //   context_switch_to_host(regs, eid, 1);
  // } else if(request == STOP_EDGE_CALL_HOST_ACCEPT){
  //   regs->a0 = 1;
  //   context_switch_to_host(regs, eid, 1);
  // } else {
  //   context_switch_to_host(regs, eid, 0);
  ...
  context_switch_to_host(regs, eid, (request == STOP_EDGE_CALL_HOST) || (request == STOP_EDGE_CALL_HOST_ACCEPT));

  switch(request) {
    case(STOP_TIMER_INTERRUPT):
      return SBI_ERR_SM_ENCLAVE_INTERRUPTED;
    case(STOP_EDGE_CALL_HOST):
    ocall_info[csr_read(CSR_MHARTID)]++;
      return SBI_ERR_SM_ENCLAVE_EDGE_CALL_HOST;
    case (STOP_EDGE_CALL_HOST_ACCEPT):
      ocall_info[csr_read(CSR_MHARTID)]++;
      sbi_printf("OCALL INFO: H0/%ld H1/%ld H2/%ld H3/%ld H4/%ld\n", ocall_info[0], ocall_info[1], ocall_info[2], ocall_info[3], ocall_info[4]);
      ocall_info[0] = 0;
      ocall_info[1] = 0;
      ocall_info[2] = 0;
      ocall_info[3] = 0;
...
unsigned long attest_enclave(uintptr_t report_ptr, uintptr_t data, uintptr_t size, enclave_id eid)
{
  ...
  for(memid=0; memid < ENCLAVE_REGIONS_MAX; memid++) {
    if(enclaves[eid].regions[memid].type != REGION_INVALID) {
      pmp_set_keystone(enclaves[eid].regions[memid].pmp_rid, EPMP_ALL_RW);
    }
  }
  ...
  // for(memid=0; memid < ENCLAVE_REGIONS_MAX; memid++) {
  //   if(enclaves[eid].regions[memid].type != REGION_INVALID) {
  //     pmp_set_keystone(enclaves[eid].regions[memid].pmp_rid, EPMP_SU_RWX);
  //   }
  // }
                                 ...
                                 size_t key_ident_size, enclave_id eid)
{
  ...
  //pmp_set_keystone(enclaves[eid].regions[0].pmp_rid, EPMP_ALL_RW);
