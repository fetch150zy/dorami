...
int aclint_mswi_cold_init(struct aclint_mswi_data *mswi)
{
	...
	struct sbi_domain_memregion reg; //TODO: Read here the target... then take it for the impl later

	sbi_printf("[OSBI] Initialiying ACLINT with: Addr: 0x%lx, size: 0x%lx, hartid: 0x%lx, hart cnt: 0x%lx\n", mswi->addr, mswi->size, mswi->first_hartid, mswi->hart_count);
