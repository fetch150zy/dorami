...
static void __noreturn __attribute__ ((noinline)) init_coldboot(struct sbi_scratch *scratch, u32 hartid)
{
	int rc;
	unsigned long *count;
	const struct sbi_platform *plat = sbi_platform_ptr(scratch);
	...
	sbi_printf("[OSBI] BASIC CONSOLE INITIALIZED\n");
	...
	sbi_printf("Boot HART MSTATUS      : %lx\n", csr_read(CSR_MSTATUS));

	sbi_printf("Boot HART MSCRATCH      : %lx\n", csr_read(CSR_MSCRATCH));
					 ...
					 u32 hartid)
{
	...
	sbi_printf("[OSBI] HART %d now proceeding with init after IPI\n", hartid);
